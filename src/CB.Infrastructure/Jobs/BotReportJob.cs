...
public class BotReportJob(IServiceProvider serviceProvider) : IInvocable {
    ...
    public async Task Invoke() {
        ...
        if (lastReport != null && lastReport.Year == now.Year && lastReport.Month == (now.Month - 1)) return; // return, if pre report has existed
        ...
        var transactions = await this.db.Transactions.Include(o => o.UserBot)
            .Where(o=> !string.IsNullOrEmpty(o.MerchantId))
            .Where(o => o.TransactionAt >= startDate && o.TransactionAt <= endDate).ToListAsync();
        ...
        var reports = transactions.GroupBy(o => new { o.MerchantId, o.TransactionAt.Year, o.TransactionAt.Month, o.UserBot!.BotId }).Select(o => {
            var balance = userBotBalance.GetValueOrDefault(o.Key.BotId);
            ...
            return new BotReport {
                ...
                Balance = balance,
                Profit = profit,
                ProfitPercent = balance == 0 ? balance : profit / balance,
            };
        }).ToList();
